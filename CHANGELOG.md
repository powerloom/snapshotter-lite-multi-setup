...
### Added
- **Custom snapshot config and compute repos (profile overrides)** - Namespaced profile `.env` may set `SNAPSHOT_CONFIG_REPO`, `SNAPSHOT_CONFIG_REPO_BRANCH`, `SNAPSHOT_CONFIG_REPO_COMMIT`, `SNAPSHOTTER_COMPUTE_REPO`, `SNAPSHOTTER_COMPUTE_REPO_BRANCH`, and `SNAPSHOTTER_COMPUTE_REPO_COMMIT`. `deployment.py` merges these after loading the profile and applies curated-datamarkets (`sources.json`) values with `setdefault`, so **operator-supplied repos, branches, and optional commit SHAs take precedence** over market defaults. (Commit IDs from `sources.json` alone were already noted in v0.3.0; this documents the **profile override** path.)
